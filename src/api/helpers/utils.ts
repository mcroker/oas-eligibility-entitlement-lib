...
import { BenefitResultsObject } from '../definitions/types'
import { LivingCountryHelper } from './fieldClasses'
import roundToTwo from './roundToTwo'

export const getDeferralIncrease = (months: number, baseAmount: number) => {
...
export function getAgeArray(residencyData: {
  ...
  function yearsUntilOAS(age: number, residency: number) {
  ...
  function yearsUntilALW(age: number, residency: number) {
...
export function AlwsEligibility(age: number, yearsInCanada: number) {
...
export function evaluateOASInput(input: OasInput) {
  let canDefer = false
  ...
  const livingCountry = new LivingCountryHelper(input.livingCountry);
 ...
 */
export function calculateAge(birthMonth: number, birthYear: number): number | null {
...
export function calculateFutureYearMonth(birthYear: number, birthMonth: number, age: number) {
